...
async function extractDidDocument(agent, connectionRecord) {
    ...
    const oldDidDocJson = untypedConnectionRecord.didDoc;
    const oldTheirDidDocJson = untypedConnectionRecord.theirDidDoc;
    if (oldDidDocJson) {
        const oldDidDoc = utils_1.JsonTransformer.fromJSON(oldDidDocJson, connections_1.DidDoc);
        agent.config.logger.debug(`Found a legacy did document for did ${oldDidDoc.id} in connection record didDoc. Converting it to a peer did document.`);
        const newDidDocument = helpers_1.convertToNewDidDocument(oldDidDoc);
        ...
        if (!didRecord) {
            ...
            didRecord = new repository_1.DidRecord({
            ...
            didRecord.metadata.set(didRecordMetadataTypes_1.DidRecordMetadataKeys.LegacyDid, {
                ...
                didDocumentString: utils_1.JsonEncoder.toString(oldDidDocJson),
            });
    ...
    if (oldTheirDidDocJson) {
        ...
        const newTheirDidDocument = helpers_1.convertToNewDidDocument(oldTheirDidDoc);
        // Maybe we already have a record for this did because the migration failed previously
...
async function migrateToOobRecord(agent, connectionRecord) {
    ...
    if (oldInvitationJson) {
        ...
        const outOfBandInvitation = helpers_2.convertToNewInvitation(oldInvitation);
        ...
        if (oldMultiUseInvitation) {
            ...
            agent.config.logger.debug(`Set reusable=true for out of band record with invitation @id ${oobRecord.outOfBandInvitation.id}.`);
            return;
        }
