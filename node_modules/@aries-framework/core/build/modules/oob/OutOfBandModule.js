...
let OutOfBandModule = OutOfBandModule_1 = class OutOfBandModule {
    constructor(dispatcher, agentConfig, outOfBandService, routingService, connectionsModule, didCommMessageRepository, messageSender, eventEmitter, didCommDocumentService) {
    ...
    async createInvitation(config = {}) {
        ...
        const routing = (_f = config.routing) !== null && _f !== void 0 ? _f : (await this.routingService.getRouting({}));
     ...
     * @returns OutOfBandInvitation
     */
    async parseInvitationShortUrl(invitation) {
        return await parseInvitation_1.parseInvitationShortUrl(invitation, this.agentConfig.agentDependencies);
    }
    /**
    ...
    async emitWithConnection(connectionRecord, messages) {
        ...
        const plaintextMessage = messages.find((message) => {
            const parsedMessageType = messageType_1.parseMessageType(message['@type']);
            return supportedMessageTypes.find((type) => messageType_1.supportsIncomingMessageType(parsedMessageType, type));
        });
        if (!plaintextMessage) {
    ...
    static register(dependencyManager) {
        // Api
        dependencyManager.registerContextScoped(OutOfBandModule_1);
        // Services
...
OutOfBandModule = OutOfBandModule_1 = __decorate([
    plugins_1.module(),
