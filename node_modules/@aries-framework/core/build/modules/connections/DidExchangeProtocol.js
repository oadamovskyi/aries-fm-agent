...
let DidExchangeProtocol = class DidExchangeProtocol {
    ...
    async processResponse(messageContext, outOfBandRecord) {
        ...
        const didDocument = await this.extractDidDocument(message, outOfBandRecord
            .getTags()
            .recipientKeyFingerprints.map((fingerprint) => dids_1.Key.fromFingerprint(fingerprint).publicKeyBase58));
        const didRecord = new repository_1.DidRecord({
            id: message.did,
            role: DidDocumentRole_1.DidDocumentRole.Received,
            didDocument,
