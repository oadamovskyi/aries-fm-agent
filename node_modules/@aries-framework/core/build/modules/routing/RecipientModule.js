...
let RecipientModule = RecipientModule_1 = class RecipientModule {
    constructor(dispatcher, agentConfig, mediationRecipientService, connectionService, dids, messageSender, eventEmitter, discoverFeaturesModule, mediationRepository, routingService) {
    ...
    async sendMessage(outboundMessage, pickupStrategy) {
        const mediatorPickupStrategy = pickupStrategy !== null && pickupStrategy !== void 0 ? pickupStrategy : this.agentConfig.mediatorPickupStrategy;
    ...
    async openWebSocketAndPickUp(mediator, pickupStrategy) {
        ...
        const stopConditions$ = rxjs_1.merge(this.agentConfig.stop$, this.stopMessagePickup$).pipe();
        ...
        operators_1.takeUntil(stopConditions$), operators_1.filter((e) => e.payload.connectionId === mediator.connectionId))
            .subscribe(() => {
    ...
    async initiateMessagePickup(mediator, pickupStrategy) {
        ...
        switch (mediatorPickupStrategy) {
            ...
            case MediatorPickupStrategy_1.MediatorPickupStrategy.PickUpV1: {
                ...
                this.agentConfig.logger.info(`Starting explicit (batch) pickup of messages from mediator '${mediatorRecord.id}'`);
                const subscription = rxjs_1.interval(mediatorPollingInterval)
                    .pipe(operators_1.takeUntil(stopConditions$))
                    .subscribe({
                    next: async () => {
                        await this.pickupMessages(mediatorConnection);
                    },
                    complete: () => this.agentConfig.logger.info(`Stopping pickup of messages from mediator '${mediatorRecord.id}'`),
                });
                return subscription;
            }
            case MediatorPickupStrategy_1.MediatorPickupStrategy.Implicit:
                ...
                break;
            default:
                this.agentConfig.logger.info(`Skipping pickup of messages from mediator '${mediatorRecord.id}' due to pickup strategy none`);
        }
    }
    /**
    ...
    async getPickupStrategyForMediator(mediator) {
        ...
        if (!mediatorPickupStrategy) {
            const isPickUpV2Supported = await this.discoverFeaturesModule.isProtocolSupported(mediator.connectionId, protocol_1.StatusRequestMessage);
            if (isPickUpV2Supported) {
                mediatorPickupStrategy = MediatorPickupStrategy_1.MediatorPickupStrategy.PickUpV2;
            }
            else {
                const isBatchPickupSupported = await this.discoverFeaturesModule.isProtocolSupported(mediator.connectionId, protocol_1.BatchPickupMessage);
    ...
    async pickupMessages(mediatorConnection, pickupStrategy) {
        ...
        const outboundMessage = helpers_1.createOutboundMessage(mediatorConnection, pickupMessage);
    ...
    async provision(connection) {
        ...
        if (!mediation) {
            ...
            this.agentConfig.logger.debug(`Mediator invitation has already been ${mediation.isReady ? 'granted' : 'requested'}`);
        }
        return mediation;
    }
    ...
    static register(dependencyManager) {
        // Api
        dependencyManager.registerContextScoped(RecipientModule_1);
        // Services
        dependencyManager.registerSingleton(MediationRecipientService_1.MediationRecipientService);
        dependencyManager.registerSingleton(RoutingService_1.RoutingService);
        // Repositories
...
RecipientModule = RecipientModule_1 = __decorate([
    plugins_1.module(),
