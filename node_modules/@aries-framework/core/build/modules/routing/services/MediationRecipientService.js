...
let MediationRecipientService = class MediationRecipientService {
    ...
    async createRequest(connection) {
        ...
        connection.setTag('connectionType', [ConnectionType_1.ConnectionType.Mediator]);
    ...
    async processMediationGrant(messageContext) {
        ...
        const connectionUsesDidKey = messageContext.message.routingKeys.some(helpers_2.isDidKey);
        await this.updateUseDidKeysFlag(connection, messages_1.MediationGrantMessage.type.protocolUri, connectionUsesDidKey);
    ...
    async processKeylistUpdateResults(messageContext) {
        ...
        // Update connection metadata to use their key format in further protocol messages
        const connectionUsesDidKey = keylist.some((key) => helpers_2.isDidKey(key.recipientKey));
        await this.updateUseDidKeysFlag(connection, messages_1.KeylistUpdateResponseMessage.type.protocolUri, connectionUsesDidKey);
        // update keylist in mediationRecord
        for (const update of keylist) {
            if (update.action === messages_1.KeylistUpdateAction.add) {
                mediationRecord.addRecipientKey(helpers_2.didKeyToVerkey(update.recipientKey));
            }
            else if (update.action === messages_1.KeylistUpdateAction.remove) {
                mediationRecord.removeRecipientKey(helpers_2.didKeyToVerkey(update.recipientKey));
            }
        }
    ...
    async keylistUpdateAndAwait(mediationRecord, verKey, timeoutMs = 15000 // TODO: this should be a configurable value in agent config
        ...
        const message = this.createKeylistUpdateMessage(useDidKey ? helpers_2.verkeyToDidKey(verKey) : verKey);
        mediationRecord.assertReady();
        mediationRecord.assertRole(models_1.MediationRole.Recipient);
    ...
    async findAllMediatorsByQuery(query) {
