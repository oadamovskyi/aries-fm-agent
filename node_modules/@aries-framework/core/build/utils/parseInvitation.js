...
const oobInvitationFromShortUrl = async (response) => {
    ...
    if (response) {
        if (((_a = response.headers.get('Content-Type')) === null || _a === void 0 ? void 0 : _a.startsWith('application/json')) && response.ok) {
            ...
            const parsedMessageType = messageType_1.parseMessageType(invitationJson['@type']);
            if (messageType_1.supportsIncomingMessageType(parsedMessageType, messages_1.OutOfBandInvitation.type)) {
                const invitation = JsonTransformer_1.JsonTransformer.fromJSON(invitationJson, messages_1.OutOfBandInvitation);
                await MessageValidator_1.MessageValidator.validateSync(invitation);
                return invitation;
            }
            else if (messageType_1.supportsIncomingMessageType(parsedMessageType, connections_1.ConnectionInvitationMessage.type)) {
            ...
            return exports.parseInvitationUrl(responseUrl);
        }
    }
...
const parseInvitationShortUrl = async (invitationUrl, dependencies) => {
    ...
    if (parsedUrl['oob']) {
        ...
        try {
            return exports.oobInvitationFromShortUrl(await fetchShortUrl(invitationUrl, dependencies));
        }
        catch (error) {
